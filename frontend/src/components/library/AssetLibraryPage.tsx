...
export default function AssetLibraryPage() {
  ...
  const tabs: { id: AssetTab; label: string; icon: typeof Users }[] = [
    { id: "characters", label: t("library.characters"), icon: Users },
    { id: "scenes", label: t("library.scenes"), icon: MapPin },
    { id: "props", label: t("library.props"), icon: Package },
  ];
  ...
  return (
    <div className="container mx-auto px-6 py-8 max-w-6xl">
      ...
      <div className="flex border-b border-gray-700/50 mb-6">
        ...
        <span className="self-center text-xs text-gray-500">{t("library.assetsCount", { count: totalCount })}</span>
      </div>

      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="text-gray-400">{t("library.loading")}</div>
        </div>
      ) : filteredSources.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-500">
          <ImageIcon size={48} className="mb-3 text-gray-600" />
          <p className="text-sm">{t("library.emptyTitle")}</p>
          <p className="text-xs text-gray-600 mt-1">{t("library.emptyHint")}</p>
        </div>
      ) : (
              ...
              <div key={source.id}>
                ...
                >
                  ...
                  <span className="text-xs px-2 py-0.5 rounded bg-white/5 text-gray-500">
                    {source.type === "series" ? t("library.series") : t("library.project")} · {assets.length}
                  </span>
                </button>
                {!isCollapsed && (
